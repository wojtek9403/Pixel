...
public class PointsNavPane extends BorderPane {
	...
	public void calculateCoordinates(javafx.scene.input.KeyEvent n) {
		...
		if (n.getCode() == KeyCode.ENTER) {
			...
			double yPositioner = YsizeOfPic / y;
			double xPositioner = XsizeOfPic / x;

			if (y < YsizeOfPic - ((this.menagedPoints.get(0).getRadius())*2)
					&& x < XsizeOfPic - ((this.menagedPoints.get(0).getRadius())*2)) {
				for (MyPoint p : this.menagedPoints) {

					System.out.println(" y: " + y + "  Ypic : " + YsizeOfPic + " x: " + x + " Xpic: " + XsizeOfPic);

					p.layoutYProperty().bind(this.getView().getWorkSpace().heightofContainers.divide(yPositioner));
					p.layoutXProperty().bind(this.getView().getWorkSpace().widthofContainers.divide(xPositioner));
